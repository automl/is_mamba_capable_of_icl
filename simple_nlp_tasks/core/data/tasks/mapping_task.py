...
class MappingTask(Task):
    ...
    def sample_inputs(self, num_inputs: int, exclude: List[str] = ()) -> List[str]:
        ...
        input_space_list = [item for item in input_space if item not in exclude]
        chosen_samples = np.random.choice(input_space_list, size=num_inputs, replace=False).tolist()
