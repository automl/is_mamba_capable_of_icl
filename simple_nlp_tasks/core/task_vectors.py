...
def run_icl(
    ...
    if batched:
        with torch.no_grad():
            ...
            new_ids = batch_generate(model, tokenizer, inputs=inputs, batch_size=1, generate_kwargs={"max_new_tokens": 1})
            predictions = decode_predictions(new_ids, tokenizer)
    else:
        predictions = []
        for test_dataset in test_datasets:
            inputs = tokenize_datasets(tokenizer, [test_dataset], format_dataset_kwargs=format_dataset_kwargs)
            new_ids = batch_generate(model, tokenizer, inputs=inputs, generate_kwargs={"max_new_tokens": 1})
            predictions.append(decode_predictions(new_ids, tokenizer)[0])
